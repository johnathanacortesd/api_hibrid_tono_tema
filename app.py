...
with st.sidebar:
    ...
    usar_limpieza_llm = st.checkbox(
        ...
        value=True,
        help="Mejora ortografía y puntuación sin alterar contenido."
    )
    
    if usar_limpieza_llm:
        modelo_limpieza = st.selectbox(
            "Modelo de limpieza:",
            options=["llama", "gpt", "claude"],
            index=0,
            format_func=lambda x: {
                "llama": "🦙 Llama 3.3 70B (Groq - Recomendado)",
                "gpt": "🤖 GPT-4o-mini (OpenAI)",
                "claude": "🧠 Claude 3.5 Haiku (Anthropic)"
            }[x],
            help="Llama usa tu API key de Groq existente"
        )
    else:
        modelo_limpieza = "llama"
    ...
    if st.button("🚀 Iniciar Transcripción", type="primary", use_container_width=True):
        ...
        try:
            ...
            with st.status("🎯 Transcribiendo audio...", expanded=True) as status:
                ...
                # Limpieza opcional
                if usar_limpieza_llm:
                    st.write(f"🧹 Limpiando texto con {modelo_limpieza.upper()}...")
                    texto_limpio = limpiar_con_llm(texto_original, True, modelo_limpieza)
                else:
