...
class ClasificadorTonoUltraV2:
    ...
    async def _clasificar_grupo_async(self, texto_representante: str, semaphore: asyncio.Semaphore):
        async with semaphore:
            ...
            is_crisis_response = bool(CRISIS_KEYWORDS.search(t)) and bool(re.search(
                rf"{brand_re}.{{0,50}}{RESPONSE_VERBS.pattern}", t, re.IGNORECASE
            ))
...
class ClasificadorTemaDinamico:
    ...
    def _generar_subtema_para_grupo(self, textos_muestra: List[str]) -> str:
        """Genera un subtema específico basado en título y resumen"""
        prompt = (
            ...
            f"El subtema debe reflejar el hecho noticioso concreto.\n\n"
        ...
        try:
            ...
            data = json.loads(resp.choices[0].message.content.strip())
            return limpiar_tema_geografico(
                limpiar_tema(data.get("subtema", "Sin tema")),
                self.marca,
                self.aliases
            )
        except Exception:
...
def consolidar_subtemas_en_temas(subtemas: List[str], p_bar) -> List[str]:
    """
    Consolida subtemas en temas principales de forma coherente.
    Máximo 30 temas principales, agrupando subtemas similares.
    """
    p_bar.progress(0.6, text="📊 Contando y filtrando subtemas...")
    subtema_counts = Counter(subtemas)
    
    # Separar subtemas frecuentes de únicos
    ...
    # Si hay muy pocos subtemas, no consolidar
    if len(subtemas_a_clusterizar) + len(singletons) <= MAX_TEMAS_PRINCIPALES:
        p_bar.progress(1.0, "ℹ️ Pocos subtemas detectados. Usando subtemas como temas.")
        for st in subtemas_a_clusterizar + singletons:
            mapa_subtema_a_tema[st] = st
        return [mapa_subtema_a_tema.get(st, st) for st in subtemas]
    
    # Calcular número óptimo de clusters
    total_subtemas = len(subtemas_a_clusterizar) + len(singletons)
    num_clusters = min(MAX_TEMAS_PRINCIPALES, max(5, total_subtemas // 3))
    
    p_bar.progress(0.7, f"🔄 Agrupando {len(subtemas_a_clusterizar)} subtemas frecuentes en {num_clusters} temas...")
    ...
    num_clusters = min(num_clusters, len(subtemas_validos))
    ...
    for cluster_id, lista_subtemas in mapa_cluster_a_subtemas.items():
        # Si el cluster tiene un solo subtema, usar ese nombre
        if len(lista_subtemas) == 1:
            tema_principal = lista_subtemas[0]
        else:
            # Generar nombre de tema que agrupe los subtemas
            prompt = (
                "Eres un analista de medios experto. A partir de estos subtemas, genera un TEMA principal (2-4 palabras) que los agrupe de forma lógica.\n"
                "El tema debe ser descriptivo y útil para un informe ejecutivo. Evita términos vagos.\n"
                "Ejemplo: Si los subtemas son 'Apertura nueva sucursal', 'Resultados financieros Q3', "
                "un buen tema es 'Expansión y Resultados Financieros'.\n\n"
    ...
    # Asignar subtemas únicos (singletons) a los temas principales existentes
        ...
        if valid_theme_names:
            ...
            for singleton in singletons:
                ...
                if emb_singleton is not None:
                    ...
                    # Solo asignar si la similitud es razonable (>0.7)
                    if best_similarity > 0.7:
                        mapa_subtema_a_tema[singleton] = valid_theme_names[best_match_idx]
                    else:
                        # Si no hay buena similitud, mantener como tema independiente
                        mapa_subtema_a_tema[singleton] = singleton
        else:
            # Si no hay temas válidos, mantener singletons como temas independientes
            for singleton in singletons:
                mapa_subtema_a_tema[singleton] = singleton
    else:
        # Si no hay temas principales, mantener singletons como están
        for singleton in singletons:
            mapa_subtema_a_tema[singleton] = singleton
