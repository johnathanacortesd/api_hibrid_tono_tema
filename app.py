...
class ClasificadorTono:
    def __init__(self, marca, aliases):
        ...
        # Lista de todas las variaciones de la marca en minúsculas
        self._all_names = [self.marca.lower()] + [a.lower() for a in self.aliases]

    def _menciona_marca(self, texto):
        """Revisa rápidamente si la marca o algún alias aparece en el texto."""
        t = unidecode(texto.lower())
        return any(m in t for m in self._all_names)

    async def _clasificar_llm(self, texto, sem):
        """Envía el texto al LLM con un prompt estrictamente enfocado en Relaciones Públicas."""
        async with sem:
            # Si ni siquiera mencionan a la marca, es Neutro. No gastamos LLM.
            if not self._menciona_marca(texto):
                return {"tono": "Neutro"}

            # Construimos el Prompt de Reputación
    ...
    async def procesar_lote_async(self, textos, pbar, resumenes, titulos):
        """Agrupa textos similares para no gastar API de más, y luego evalúa tono reputacional."""
        n = len(textos)
        txts = textos.tolist()
        ...
        # Enviar solo los representantes al LLM
        ...
        # Replicar el resultado al resto del grupo
