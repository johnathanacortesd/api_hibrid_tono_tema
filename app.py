...
# Similitud mínima entre el embedding del TEXTO de la noticia y el embedding
# de su SUBTEMA asignado. Por debajo de este valor se reclasifica individualmente.
# Valor bajo (0.35) porque texto largo vs frase corta tiene similitud moderada
# de por sí; solo rechaza cuando no hay relación semántica real.
...
def limpiar_tema_geografico(tema, marca, aliases):
    if not tema: return "Sin tema"
    palabras_originales = tema.split(); eliminar = set()
...
def _validar_estructura_subtema(etiqueta: str) -> bool:
    ...
    if len(etiqueta.split()) > 6: return False
...
def _fusionar_subtemas_semanticos(subtemas, textos_por_subtema, marca, aliases, umbral=UMBRAL_FUSION_SUBTEMAS):
    ...
    for sub in unique_subs:
        txts = textos_por_subtema.get(sub, []); palabras = []
        for t in txts[:20]:
...
class ClasificadorSubtema:
    ...
    def procesar_lote(self, col, pbar, res_puros, tit_puros):
        ...
        for k, (lid, idxs) in enumerate(sg):
            ...
            if len(idxs) > MAX_GRUPO_ETIQUETA:
                ...
                for sg_ in subgrupos:
                    e = self._generar_etiqueta([textos[i] for i in sg_], [titulos[i] for i in sg_], [resumenes[i] for i in sg_])
        ...
        # ── Fase 4b · Coherencia etiqueta ↔ texto fuente ────────────────────
        # Verifica que cada noticia tenga relación semántica real con su subtema.
        # Si la similitud es menor a UMBRAL_COHERENCIA_ETIQUETA se reclasifica
        # individualmente, ignorando el grupo al que fue asignada.
    ...
    def _consistencia(self, subtemas, ae, pbar):
        ...
        for sub in [s for s in centroids if len(ps[s]) >= 3]:
            ...
            for k, (oi, sv) in enumerate(zip(v_i, sims)):
                ...
                if bs != sub: r[oi] = bs
                elif sv < UMBRAL_MIN_PERTENENCIA_SUBTEMA: r[oi] = "_RECLASIFICAR"
        return r
...
def consolidar_temas(subtemas, textos, pbar):
    ...
    if len(us) <= 1:
        pbar.progress(1.0, "Un tema"); return [capitalizar_etiqueta(s) for s in subtemas]
    ...
    if len(vs) < 2:
        pbar.progress(1.0, "Sin agrupación"); return [capitalizar_etiqueta(s) for s in subtemas]
    ...
    cl = AgglomerativeClustering(n_clusters=None, distance_threshold=1-UMBRAL_TEMA,
        metric='precomputed', linkage='average').fit(dist_matrix)
    ...
    tf_validado: List[str] = []; n_forzadas = 0
    for i, (sub, tema_asignado) in enumerate(zip(subtemas, tf_inicial)):
        emb = ae[i]
        if emb is not None and tema_asignado in tema_centroids:
            sim = cosine_similarity(np.array(emb).reshape(1,-1),
                tema_centroids[tema_asignado].reshape(1,-1))[0][0]
            ...
            if st.form_submit_button("Analizar", use_container_width=True, type="primary"):
                if not bn: st.error("Indica la marca.")
                    ...
                    with st.spinner("Procesando..."):
                        st.session_state.quick_result = asyncio.run(run_quick_async(st.session_state.quick_df.copy(), tc, sc, bn, al))
                    st.rerun()
