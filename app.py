...
# Patrones positivos contextuales (cerca de la marca)
POS_BRAND_PATTERNS = [
...
def _setup_openai_key():
    """Configura la API key según la versión."""
...
def _count_usage(resp):
    """Extrae y acumula usage de respuesta (compatible v0 y v1)."""
    if resp is None:
        return
    if OPENAI_NEW_API:
...
def _create_embedding_sync(input_texts: List[str]):
    """Crea embeddings de forma síncrona, compatible con ambas APIs."""
    if OPENAI_NEW_API:
...
def _create_chat_sync(messages: List[Dict], max_tokens: int = 50, temperature: float = 0.0, response_format=None):
    """Crea chat completion síncrono, compatible con ambas APIs."""
    kwargs = {
...
async def _create_chat_async(messages: List[Dict], max_tokens: int = 50, temperature: float = 0.0, response_format=None):
    """Crea chat completion asíncrono, compatible con ambas APIs."""
    kwargs = {
        "model": OPENAI_MODEL_CLASIFICACION,
        "messages": messages,
        "max_tokens": max_tokens,
...
def _get_chat_content(resp) -> str:
    """Extrae el contenido del mensaje de respuesta."""
    if OPENAI_NEW_API:
        return resp.choices[0].message.content.strip()
    else:
        return resp.choices[0].message.content.strip() if hasattr(resp.choices[0].message, 'content') else resp["choices"][0]["message"]["content"].strip()


def _get_embedding_data(resp) -> list:
    """Extrae los datos de embedding de la respuesta."""
    if OPENAI_NEW_API:
        return resp.data
    else:
...
class ClasificadorTonoV8:
    ...
    - Propagación garantizada dentro de grupos similares
    """
    ...
    def _analyze_brand_sentiment_rules(self, contextos: List[str]) -> Optional[str]:
        """Análisis por reglas centrado en la relación marca-acción."""
        pos_score = 0
        neg_score = 0
    ...
    async def _llm_classify_tone(self, contextos: List[str], semaphore: asyncio.Semaphore) -> str:
        """Clasificación de tono por LLM con prompt mejorado."""
        async with semaphore:
            aliases_str = ", ".join(self.aliases) if self.aliases else "ninguno"
            contextos_texto = "\n---\n".join(c[:500] for c in contextos[:3])

            prompt = f"""Eres un analista de reputación corporativa. Analiza el SENTIMIENTO específicamente hacia la marca '{self.marca}' (alias: {aliases_str}) en los siguientes fragmentos.
...
CRITERIOS:
- Positivo: logros, lanzamientos, reconocimientos, alianzas, crecimiento, respuestas positivas ante crisis, innovación, apoyo a causas
- Negativo: críticas, sanciones, multas, demandas, pérdidas, fallos, quejas, escándalos
- Neutro: menciones informativas sin juicio claro, o el evento no impacta directamente su reputación

EJEMPLOS:
- "X lanzó su nueva plataforma digital" → Positivo
- "X fue multada por la SIC" → Negativo
- "En la reunión participó X junto a otros actores" → Neutro
- "Tras la emergencia, X activó su plan de contingencia" → Positivo
...
Responde SOLO en JSON: {{"tono":"Positivo"|"Negativo"|"Neutro"}}"""
            ...
            try:
                ...
                _count_usage(resp)
                content = _get_chat_content(resp)
    ...
    async def _classify_group(self, texto_representante: str, semaphore: asyncio.Semaphore) -> str:
    ...
    async def procesar_lote_async(
        ...
        progress_bar.progress(0.15, text=f"📊 {len(comp)} grupos. Analizando tono...")
...
class ClasificadorSubtemaV8:
    ...
    def _preagrupar_identicos(self, textos, titulos, resumenes):
        ...
        for i, titulo in enumerate(titulos):
            ...
            if norm:
                titulo_hash_index[hashlib.md5(norm.encode()).hexdigest()].append(i)

        for i, resumen in enumerate(resumenes):
            norm = normalizar_rapido(resumen)
            if norm:
                resumen_hash_index[hashlib.md5(norm[:150].encode()).hexdigest()].append(i)
    ...
    def _clustering_por_lotes(self, textos, titulos, indices):
        ...
        for batch_start in range(0, len(indices), BATCH_SIZE):
            ...
            batch_texts = [f"{titulos[i][:150]} {textos[i][:1200]}" for i in batch_idxs]
    ...
    def _generar_subtema(self, textos_muestra, titulos_muestra):
        titles_normalized = sorted(
            normalize_title_for_comparison(t) for t in titulos_muestra[:5]
        )
        cache_key = hashlib.md5("|".join(titles_normalized).encode()).hexdigest()
        ...
        for t in titulos_muestra[:5]:
            words = [w for w in string_norm_label(t).split() if w not in STOPWORDS_ES and len(w) > 3]
            all_words.extend(words)
        keywords = [w for w, _ in Counter(all_words).most_common(8)]

        titulos_display = "\n".join(f"- {t[:120]}" for t in titulos_muestra[:5])
...
Responde SOLO en JSON: {{"subtema": "..."}}"""
        ...
        try:
            ...
            genericos = {"sin tema", "varios", "noticias", "actividad", "gestión", "información", "nota", "noticia"}
    ...
    def _fusionar_subtemas_similares(self, subtemas, textos):
        ...
        for cluster_id in set(clustering.labels_):
            cluster_indices = [i for i, x in enumerate(clustering.labels_) if x == cluster_id]
            labels_in_cluster = [valid_labels[i] for i in cluster_indices]
            counts = {lbl: sum(1 for s in subtemas if s == lbl) for lbl in labels_in_cluster}
            representante = max(labels_in_cluster, key=lambda x: (counts.get(x, 0), -len(x)))
    ...
    def procesar_lote(self, textos_concat, progress_bar, resumen_puro, titulos_puros):
        ...
        comp = dsu.components()
        indices_sueltos = [idxs[0] for idxs in comp.values() if len(idxs) == 1]

        progress_bar.progress(0.15, f"📊 {len(grupos_rapidos)} grupos rápidos. Refinando...")

        if len(indices_sueltos) > 1:
            grupos_cluster = self._clustering_por_lotes(textos, titulos, indices_sueltos)
        ...
        subtemas_fusionados = self._fusionar_subtemas_similares(subtemas_brutos, textos)
...
# ======================================
# Consolidación de Subtemas en Temas
# ======================================
def consolidar_subtemas_en_temas(subtemas, textos, p_bar, marca="", aliases=None):
    ...
    for cid, lista_subt in clusters_contenidos.items():
        ...
        for subt in lista_subt[:3]:
            rows_subt = df_temas[df_temas['subtema'] == subt].head(2)
            for _, row_subt in rows_subt.iterrows():
                primera_oracion = str(row_subt['texto']).split('.')[0][:100]
...
Responde SOLO el nombre, sin JSON ni comillas."""
        ...
        try:
            ...
            _count_usage(resp)
            content = _get_chat_content(resp)
            nombre_tema = limpiar_tema(content.replace('"', '').replace('.', ''))
            if not nombre_tema or nombre_tema.lower() in {"sin tema", "varios", "noticias"}:
                nombre_tema = lista_subt[0]
        except Exception:
            nombre_tema = lista_subt[0]

        if marca:
            nombre_tema = limpiar_tema_geografico(nombre_tema, marca, aliases)
    ...
    if rows_to_analyze:
        ...
        with st.status("🏷️ **Paso 4/5:** Análisis de Tema y Subtema", expanded=True) as s:
            ...
            if "Solo Modelos PKL" in analysis_mode:
                ...
                clasif_subtemas = ClasificadorSubtemaV8(brand_name, brand_aliases)
...
def render_quick_analysis_tab():
    ...
    st.info("Análisis avanzado de Tono, Tema y Subtema centrado en la marca.")
    ...
    if 'quick_analysis_result' in st.session_state:
        ...
        st.download_button(
            label="📥 **Descargar Resultados**", data=excel_data,
    ...
    if 'quick_analysis_df' not in st.session_state:
        ...
        with st.form("quick_analysis_form"):
            ...
            st.caption("Si subes un PKL que contenga la marca, el campo se autocompletará.")

            pkl_for_brand = st.file_uploader(
                "PKL para detectar marca (opcional)", type=["pkl"], key="quick_pkl_brand"
            )
            ...
            brand_name = st.text_input("**Marca Principal**", value=default_brand, placeholder="Ej: Siemens")
...
def main():
    ...
    with tab1:
        if not st.session_state.get("processing_complete", False):
            ...
            c2.markdown(
                f'<div class="metric-card"><div class="metric-value" style="color:green;">'
                f'{st.session_state.unique_rows}</div>'
                f'<div class="metric-label">Únicas</div></div>',
                unsafe_allow_html=True
            )
            c3.markdown(
                f'<div class="metric-card"><div class="metric-value" style="color:orange;">'
                f'{st.session_state.duplicates}</div>'
                f'<div class="metric-label">Duplicados</div></div>',
                unsafe_allow_html=True
            )
            c4.markdown(
                f'<div class="metric-card"><div class="metric-value" style="color:blue;">'
                f'{st.session_state.process_duration}</div>'
                f'<div class="metric-label">Tiempo</div></div>',
                unsafe_allow_html=True
            )
            c5.markdown(
