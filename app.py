...
:root {
    --primary: #0f172a; --accent: #3b82f6; --accent2: #10b981; --danger: #ef4444;
    --surface: #1e293b; --surface2: #334155; --text: #f1f5f9; --text-muted: #94a3b8;
    --border: #334155; --radius: 12px; --shadow: 0 4px 24px rgba(0,0,0,0.4);
}
html, body, [class*="css"] { font-family: 'DM Sans', sans-serif !important; }
.stApp { background: var(--primary); color: var(--text); }
.main-header {
    background: linear-gradient(135deg, #1e3a5f 0%, #0f2027 100%);
    border: 1px solid #1e40af; color: white; padding: 2.5rem 2rem;
    border-radius: var(--radius); text-align: center; font-size: 2.2rem;
    font-weight: 700; margin-bottom: 1rem;
    box-shadow: 0 0 40px rgba(59,130,246,0.2); letter-spacing: -0.5px;
}
.subtitle { text-align: center; color: var(--text-muted); font-size: 0.9rem;
    margin: -0.5rem 0 2rem 0; font-family: 'DM Mono', monospace; }
.metric-card { background: var(--surface); padding: 1.4rem 1rem; border-radius: var(--radius);
...
.success-card { background: linear-gradient(135deg, #064e3b 0%, #065f46 100%);
    padding: 1.5rem; border-radius: var(--radius); border: 1px solid var(--accent2);
...
class ClasificadorSubtemaV4:
    ...
    def _generar_etiqueta(self, textos_muestra, titulos_muestra) -> str:
        ...
        prompt = f"""Eres un editor periodístico. Genera un SUBTEMA conciso (entre 2 y 5 palabras) que agrupe estas noticias.

TITULOS DE MUESTRA:
{chr(10).join([f'- {str(t)[:100]}' for t in titulos_muestra[:5]])}

PALABRAS CLAVE FRECUENTES: {keywords}

REGLAS ESTRICTAS:
1. Sé ESPECIFICO: "Creditos de Vivienda" es mejor que "Productos".
2. NO usar ciudades, paises, ni regionalismos.
{marca_regla}
4. NO usar verbos vagos como "informó", "señaló", "publicó".
5. Entre 2 y 5 palabras. Empieza con sustantivo o adjetivo.
6. BUENOS EJEMPLOS: "Fusion Bancaria Regional", "Alza Tasas Interes", "Programa RSE Ambiental", "Lanzamiento App Movil", "Crisis Liquidez Sector".
...
def render_quick_analysis_tab():
    ...
    if 'quick_analysis_result' in st.session_state:
        ...
        with col1:
            st.markdown(f'<div class="metric-card"><div class="metric-value" style="color:#ef4444;">{cost}</div><div class="metric-label">Costo Estimado</div></div>', unsafe_allow_html=True)
...
def main():
    ...
    st.markdown('<div class="main-header">📰 Sistema de Analisis de Noticias con IA</div>', unsafe_allow_html=True)
    st.markdown('<div class="subtitle">v8.0 · Tono contextual por marca · Temas 2-5 palabras · Propagacion en grupos similares · Marca opcional</div>', unsafe_allow_html=True)
    ...
    with tab1:
        if not st.session_state.get("processing_complete", False):
            ...
            for col, val, label, color in [
                (c1, st.session_state.total_rows, "Total", "#3b82f6"),
                (c2, st.session_state.unique_rows, "Unicas", "#10b981"),
                (c3, st.session_state.duplicates, "Duplicados", "#f59e0b"),
                (c4, st.session_state.process_duration, "Tiempo", "#8b5cf6"),
                (c5, st.session_state.get("process_cost", "$0.00"), "Costo Est.", "#ef4444"),
            ]:
                col.markdown(
