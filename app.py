"""
======================================
Sistema de Análisis de Noticias con IA v4.8
Autor: Johnathan Cortés
Mejoras: Agrupación coherente de Temas y Subtemas
======================================
"""
...
WINDOW = 80
MAX_TEMAS_PRINCIPALES = 30  # Máximo de temas principales
MIN_SUBTEMAS_PARA_AGRUPAR = 30  # Si hay menos subtemas únicos, no agrupar
UMBRAL_SIMILITUD_SINGLETON = 0.75  # Umbral para asignar singletons a temas

# Lista de ciudades y gentilicios colombianos
CIUDADES_COLOMBIA = {
    ...
    "santa marta", "santamarta", "quibdó", "quibdo", "leticia", "mocoa", "mitú",
    "mitu", "puerto carreño", "inírida", "inirida", "san josé del guaviare",
    "antioquia", "atlántico", "atlantico", "bolívar", "bolivar", "boyacá", "boyaca",
    "caldas", "caquetá", "caqueta", "casanare", "cauca", "cesar", "chocó", "choco",
    "córdoba", "cordoba", "cundinamarca", "guainía", "guainia", "guaviare", "huila",
    "la guajira", "magdalena", "meta", "nariño", "narino", "norte de santander",
    "putumayo", "quindío", "quindio", "risaralda", "san andrés", "san andres",
...
GENTILICIOS_COLOMBIA = {
    ...
    "capitalina", "capitalinas", "antioqueño", "antioqueños", "antioqueña", "antioqueñas",
    "paisa", "paisas", "medellense", "medellenses", "caleño", "caleños", "caleña",
    "caleñas", "valluno", "vallunos", "valluna", "vallunas", "vallecaucano",
    "vallecaucanos", "barranquillero", "barranquilleros", "cartagenero", "cartageneros",
    "costeño", "costeños", "costeña", "costeñas", "cucuteño", "cucuteños", "bumangués",
    "santandereano", "santandereanos", "boyacense", "boyacenses", "tolimense",
    "tolimenses", "huilense", "huilenses", "nariñense", "nariñenses", "pastuso",
    "pastusas", "cordobés", "cordobeses", "cauca", "caucano", "caucanos", "chocoano",
    "chocoanos", "casanareño", "casanareños", "caqueteño", "caqueteños", "guajiro",
    "guajiros", "llanero", "llaneros", "amazonense", "amazonenses", "colombiano",
    "colombianos", "colombiana", "colombianas"
}

# ======================================
# Léxicos y Patrones para Análisis de Tono
# ======================================
STOPWORDS_ES = set("""
a ante bajo cabe con contra de desde durante en entre hacia hasta mediante para por
segun sin so sobre tras y o u e la el los las un una unos unas lo al del se su sus
le les mi mis tu tus nuestro nuestros vuestra vuestras este esta estos estas ese esa
esos esas aquel aquella aquellos aquellas que cual cuales quien quienes cuyo cuya
cuyos cuyas como cuando donde cual es son fue fueron era eran sera seran seria serian
he ha han habia habian hay hubo habra habria estoy esta estan estaba estaban estamos
estan estar estare estaria estuvieron estarian estuvo asi ya mas menos tan tanto cada
""".split())
...
def limpiar_tema_geografico(tema: str, marca: str, aliases: List[str]) -> str:
    ...
    # Remover frases geográficas
    frases_geograficas = [
        "en colombia", "de colombia", "del pais", "en el pais", "nacional",
...
class ClasificadorTonoUltraV2:
    ...
    async def _llm_refuerzo(self, texto: str) -> Dict[str, str]:
        ...
        try:
            ...
            return {
                ...
                "justificacion": data.get("justificacion", "Análisis LLM")
            }
        except Exception:
    ...
    async def _clasificar_grupo_async(self, texto_representante: str, semaphore: asyncio.Semaphore):
        async with semaphore:
            ...
            pos_hits = sum(1 for p in POS_PATTERNS if re.search(
                rf"{brand_re}.{{0,{WINDOW}}}{p.pattern}|{p.pattern}.{{0,{WINDOW}}}{brand_re}",
                t, re.IGNORECASE
            ))
            neg_hits = sum(1 for p in NEG_PATTERNS if re.search(
                rf"{brand_re}.{{0,{WINDOW}}}{p.pattern}|{p.pattern}.{{0,{WINDOW}}}{brand_re}",
                t, re.IGNORECASE
            ))
            
            is_crisis_response = bool(CRISIS_KEYWORDS.search(t)) and bool(
                re.search(rf"{brand_re}.{{0,50}}{RESPONSE_VERBS.pattern}", t, re.IGNORECASE)
            )
            
            if is_crisis_response:
                return {"tono": "Positivo", "justificacion": "Respuesta activa a crisis"}
            if pos_hits > neg_hits and pos_hits > 0:
                return {"tono": "Positivo", "justificacion": "Acción favorable"}
            if neg_hits > pos_hits and neg_hits > 0:
                return {"tono": "Negativo", "justificacion": "Hecho adverso"}
    ...
    async def procesar_lote_async(
        ...
        representantes = {
            cid: seleccionar_representante(idxs, textos)[1]
            for cid, idxs in comp.items()
        }
        
        semaphore = asyncio.Semaphore(CONCURRENT_REQUESTS)
        tasks = [
            self._clasificar_grupo_async(rep_texto, semaphore)
            for rep_texto in representantes.values()
        ]
        ...
        for i, f in enumerate(asyncio.as_completed(tasks)):
            ...
            progress_bar.progress(
                ...
                text=f"🎯 Analizando tono: {i+1}/{len(tasks)}"
            )
        
        resultados_por_grupo = {
            list(representantes.keys())[i]: res
...
def analizar_tono_con_pkl(textos: List[str], pkl_file: io.BytesIO) -> Optional[List[Dict[str, str]]]:
    try:
        ...
        resultados = [
            {"tono": TONO_MAP.get(p, str(p).title()), "justificacion": "Análisis con modelo PKL"}
...
# ======================================
# Clasificador de Temas (IA y PKL) - MEJORADO
# ======================================
class ClasificadorTemaDinamico:
    ...
    def _generar_subtema_para_grupo(self, textos_muestra: List[str]) -> str:
        """Genera un subtema específico basado en título + resumen"""
        prompt = (
            f"Genera un subtema específico y preciso (2-6 palabras) para estas noticias. "
            f"No incluyas la marca '{self.marca}', ciudades o gentilicios de Colombia.\n"
            f"Enfócate en la ACCIÓN o EVENTO específico mencionado.\n\n"
        ...
        try:
            ...
            data = json.loads(resp.choices[0].message.content.strip())
            subtema_raw = data.get("subtema", "Sin tema")
            return limpiar_tema_geografico(limpiar_tema(subtema_raw), self.marca, self.aliases)
        except Exception:
            # Fallback: extraer palabras clave del texto
            palabras_clave = " ".join(string_norm_label(" ".join(textos_muestra)).split()[:4])
            return limpiar_tema(palabras_clave or "Actividad Empresarial")
    ...
    def procesar_lote(
        ...
        """Genera subtemas específicos para cada grupo de noticias similares"""
        ...
        # Agrupar noticias similares usando DSU
        class DSU:
            def __init__(self, n):
                self.p = list(range(n))
        ...
        dsu = DSU(n)
        for g in [
            agrupar_textos_similares(textos, SIMILARITY_THRESHOLD_TEMAS),
            agrupar_por_titulo_similar(titulos_puros.tolist()),
            agrupar_por_resumen_puro(resumen_puro.tolist())
        ]:
            for _, idxs in g.items():
        ...
        # Generar subtemas para cada grupo
        mapa_idx_a_subtema = {}
        total_comp = len(comp)
        ...
        for hechos, (cid, idxs) in enumerate(comp.items(), 1):
            ...
            for i in idxs:
                mapa_idx_a_subtema[i] = subtema
            progress_bar.progress(
                0.1 + 0.5 * hechos / max(total_comp, 1),
                f"🏷️ Subtemas creados: {hechos}/{total_comp}"
            )
...
def consolidar_subtemas_en_temas(subtemas: List[str], p_bar) -> List[str]:
    """
    Consolida subtemas en temas principales de forma inteligente.
    
    Lógica mejorada:
    1. Si hay < MIN_SUBTEMAS_PARA_AGRUPAR subtemas únicos → tema = subtema
    2. Si hay >= MIN_SUBTEMAS_PARA_AGRUPAR → agrupar en máximo MAX_TEMAS_PRINCIPALES
    3. Subtemas únicos (singletons) se asignan al tema más similar si superan umbral
    """
    p_bar.progress(0.6, text="📊 Contando y filtrando subtemas...")
    
    subtema_counts = Counter(subtemas)
    subtemas_unicos = [st for st in subtema_counts.keys() if st != "Sin tema"]
    num_subtemas_unicos = len(subtemas_unicos)
    
    st.write(f"📌 **{num_subtemas_unicos} subtemas únicos detectados**")
    
    # Caso 1: Pocos subtemas → No agrupar
    if num_subtemas_unicos < MIN_SUBTEMAS_PARA_AGRUPAR:
        p_bar.progress(1.0, f"ℹ️ Menos de {MIN_SUBTEMAS_PARA_AGRUPAR} subtemas únicos. Tema = Subtema.")
        return subtemas  # Tema = Subtema
    
    # Caso 2: Muchos subtemas → Agrupar
    st.write(f"🔄 Agrupando {num_subtemas_unicos} subtemas en máximo {MAX_TEMAS_PRINCIPALES} temas...")
    
    # Separar subtemas frecuentes (>1 ocurrencia) de singletons (1 ocurrencia)
    subtemas_frecuentes = [st for st, count in subtema_counts.items() if st != "Sin tema" and count > 1]
    singletons = [st for st, count in subtema_counts.items() if st != "Sin tema" and count == 1]
    
    mapa_subtema_a_tema = {}
    mapa_subtema_a_tema["Sin tema"] = "Sin tema"
    
    # Si no hay suficientes subtemas frecuentes, incluir singletons
    if len(subtemas_frecuentes) < 10:
        subtemas_a_clusterizar = subtemas_unicos
    else:
        subtemas_a_clusterizar = subtemas_frecuentes
    
    if len(subtemas_a_clusterizar) < 2:
        p_bar.progress(1.0, "ℹ️ No hay suficientes subtemas para agrupar.")
        return subtemas
    
    p_bar.progress(0.7, f"🔄 Calculando embeddings para {len(subtemas_a_clusterizar)} subtemas...")
    ...
    if len(subtemas_validos) < 2:
        p_bar.progress(1.0, "ℹ️ No hay suficientes embeddings válidos.")
        return subtemas
    
    # Determinar número óptimo de clusters
    num_clusters = min(MAX_TEMAS_PRINCIPALES, max(2, len(subtemas_validos) // 3))
    st.write(f"🎯 Creando **{num_clusters} temas principales**")
    ...
    # Generar nombres de temas principales
    mapa_temas_finales = {}
    for cluster_id, lista_subtemas in mapa_cluster_a_subtemas.items():
        prompt = (
            "Eres un analista de medios experto. A partir de estos subtemas, genera un TEMA principal (2-4 palabras) que los agrupe lógicamente.\n"
            "El tema debe ser descriptivo y útil para un informe ejecutivo. Evita términos vagos.\n"
            "Ejemplo: Si los subtemas son 'Apertura nueva sucursal', 'Resultados financieros Q3' → Tema: 'Expansión y Resultados Financieros'\n\n"
            f"Subtemas: {', '.join(lista_subtemas[:12])}\n\n"
            "Responde solo con el nombre del TEMA principal (sin comillas)."
        )
        try:
            resp = call_with_retries(
                openai.ChatCompletion.create,
            ...
            # Fallback: usar el subtema más largo
    ...
    # Asignar singletons al tema más similar (si supera umbral)
    if singletons and mapa_temas_finales:
        p_bar.progress(0.9, "✨ Asignando subtemas únicos a temas principales...")
        
        temas_unicos = list(set(mapa_temas_finales.values()))
        emb_temas = {tema: get_embedding(tema) for tema in temas_unicos}
        temas_validos = [tema for tema, emb in emb_temas.items() if emb]
        
        if temas_validos:
            emb_theme_matrix = np.array([emb_temas[tema] for tema in temas_validos])
            
            for singleton in singletons:
                emb_singleton = get_embedding(singleton)
                if emb_singleton is not None:
                    sims = cosine_similarity([emb_singleton], emb_theme_matrix)[0]
                    max_sim = np.max(sims)
                    
                    # Solo asignar si la similitud supera el umbral
                    if max_sim >= UMBRAL_SIMILITUD_SINGLETON:
                        best_match_idx = np.argmax(sims)
                        mapa_subtema_a_tema[singleton] = temas_validos[best_match_idx]
                    else:
                        # Si no hay similitud suficiente, el singleton es su propio tema
                        mapa_subtema_a_tema[singleton] = singleton
    
    # Mapear todos los subtemas a sus temas
    temas_finales = []
    for subtema in subtemas:
        if subtema in mapa_subtema_a_tema:
            temas_finales.append(mapa_subtema_a_tema[subtema])
        else:
            # Si no está mapeado, usar el subtema como tema
            temas_finales.append(subtema)
    
    num_temas_finales = len(set(temas_finales) - {"Sin tema"})
    st.write(f"✅ **{num_temas_finales} temas principales generados**")
...
def detectar_duplicados_avanzado(rows: List[Dict], key_map: Dict[str, str]) -> List[Dict]:
    ...
    # Detectar duplicados por título similar en Internet
...
def generate_output_excel(all_processed_rows, key_map):
    ...
    final_order = [
        ...
        "Duracion - Nro. Caracteres", "CPE", "Tier", "Audiencia", "Tono", "Tono AI",
        "Tema", "Subtema", "Resumen - Aclaracion", "Link Nota", "Link (Streaming - Imagen)",
        "Menciones - Empresa", "Justificacion Tono", "ID duplicada"
    ]
    
    numeric_columns = {
        "ID Noticia", "Nro. Pagina", "Dimension", "Duracion - Nro. Caracteres",
        "CPE", "Tier", "Audiencia"
    }
...
# Paso 1: Limpieza y duplicados
    ...
    # Paso 2: Mapeos y Normalización
    with st.status("🗺️ **Paso 2/5:** Mapeos y Normalización", expanded=True) as s:
        df_region = pd.read_excel(region_file)
        region_map = {
            str(k).lower().strip(): v
            for k, v in pd.Series(df_region.iloc[:, 1].values, index=df_region.iloc[:, 0]).to_dict().items()
        }
        
        df_internet = pd.read_excel(internet_file)
        internet_map = {
            str(k).lower().strip(): v
            for k, v in pd.Series(df_internet.iloc[:, 1].values, index=df_internet.iloc[:, 0]).to_dict().items()
        }
        ...
        for row in all_processed_rows:
            ...
            # 2. Mapear y normalizar el medio de Internet
    ...
    # Filtrar noticias no duplicadas
    ...
    if rows_to_analyze:
        ...
        with st.status("🎯 **Paso 3/5:** Análisis de Tono", expanded=True) as s:
            ...
            positivos = tonos.get("Positivo", 0)
            negativos = tonos.get("Negativo", 0)
            neutros = tonos.get("Neutro", 0)
            
            st.markdown(
                f'**Resultados de Tono:** '
                f'<span style="color:green;">{positivos} Positivos</span>, '
                f'<span style="color:red;">{negativos} Negativos</span>, '
                f'<span style="color:gray;">{neutros} Neutros</span>',
                unsafe_allow_html=True
            )
            s.update(label="✅ **Paso 3/5:** Tono Analizado", state="complete")
        
        # Paso 4: Análisis de Tema y Subtema
        with st.status("🏷️ **Paso 4/5:** Análisis de Tema y Subtema", expanded=True) as s:
            p_bar = st.progress(0)
            
            # Generar Subtemas (siempre con IA)
            ...
            subtemas = clasif_temas.procesar_lote(
                df_temp["resumen_api"],
                p_bar,
                df_temp[key_map["resumen"]],
                df_temp[key_map["titulo"]]
            ...
            if tema_pkl_file:
                ...
                if temas_principales is None:
                    st.stop()
                df_temp[key_map["tema"]] = temas_principales
            else:
                st.write(f"🤖 Consolidando Subtemas en Temas principales con IA...")
                temas_principales = consolidar_subtemas_en_temas(subtemas, p_bar)
                df_temp[key_map["tema"]] = temas_principales
            
            num_temas = len(set(df_temp[key_map["tema"]]) - {"Sin tema", "Duplicada"})
            num_subtemas = len(set(df_temp[key_map["subtema"]]) - {"Sin tema", "Duplicada"})
            
            st.success(f"✅ **{num_temas} temas principales** y **{num_subtemas} subtemas únicos** identificados")
            s.update(label="✅ **Paso 4/5:** Temas y Subtemas Identificados", state="complete")
...
def main():
    ...
    st.markdown('<div class="subtitle">Análisis personalizable de Tono y Tema/Subtema v4.8</div>', unsafe_allow_html=True)
    ...
    if not st.session_state.get("processing_complete", False):
        ...
        c1.markdown(
            f'<div class="metric-card"><div class="metric-value">{st.session_state.total_rows}</div>'
            f'<div class="metric-label">📰 Total Noticias</div></div>',
            unsafe_allow_html=True
        )
        c2.markdown(
            f'<div class="metric-card"><div class="metric-value" style="color: #28a745;">'
            f'{st.session_state.unique_rows}</div><div class="metric-label">✅ Únicas</div></div>',
            unsafe_allow_html=True
        )
        c3.markdown(
            f'<div class="metric-card"><div class="metric-value" style="color: #ff7f0e;">'
            f'{st.session_state.duplicates}</div><div class="metric-label">🔄 Duplicados</div></div>',
            unsafe_allow_html=True
        )
        c4.markdown(
            f'<div class="metric-card"><div class="metric-value" style="color: #1f77b4;">'
            f'{st.session_state.process_duration}</div><div class="metric-label">⏱️ Duración</div></div>',
            unsafe_allow_html=True
        )
    ...
    st.markdown(
        "<hr><div style='text-align:center;color:#666;font-size:0.9rem;'>"
        "<p>Sistema de Análisis de Noticias v4.8 | Realizado por Johnathan Cortés</p></div>",
        unsafe_allow_html=True
    )
