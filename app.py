...
def check_password() -> bool:
    ...
    with col2:
        with st.container():
            ...
            st.markdown("Esta plataforma utiliza IA avanzada para análisis de tono y clasificación de temas.")
...
def jaccard(a: str, b: str) -> float:
    ta, tb = set(string_norm_label(a).split()), set(string_norm_label(b).split())
...
def _token_hash(token: str) -> int:
    return int(hashlib.md5(token.encode("utf-8")).hexdigest(), 16) & ((1 << 64) - 1)
...
def analizar_contexto_tono(texto: str, marca: str, aliases: List[str]) -> Dict[str, Any]:
    t = unidecode((texto or "").lower())
    brand_re = _build_brand_regex(marca, aliases)
    
    pos_hits = sum(1 for p in POS_PATTERNS if re.search(rf"\b{brand_re}\b.{{0,{WINDOW}}}{p.pattern}", t, re.IGNORECASE) or re.search(rf"{p.pattern}.{{0,{WINDOW}}}\b{brand_re}\b", t, re.IGNORECASE))
    neg_hits = sum(1 for p in NEG_PATTERNS if re.search(rf"\b{brand_re}\b.{{0,{WINDOW}}}{p.pattern}", t, re.IGNORECASE) or re.search(rf"{p.pattern}.{{0,{WINDOW}}}\b{brand_re}\b", t, re.IGNORECASE))
    ...
    hedging = sum(1 for w in VERBOS_DECLARATIVOS + MARCADORES_CONDICIONALES if w in t)
    
    polarity_score = pos_hits - neg_hits
    if polarity_score > 0: polarity = "Positivo"
    elif polarity_score < 0: polarity = "Negativo"
    else: polarity = "Neutro"
    
    return {"polarity": polarity, "is_crisis_response": is_crisis_response, "hedging": hedging}

def decidir_tono(features: Dict[str, Any]) -> Tuple[str, str]:
    if features.get("is_crisis_response"):
        return "Positivo", "Respuesta activa a crisis"
    if features["polarity"] == "Positivo":
        return "Positivo", "Acción favorable de la marca"
    if features["polarity"] == "Negativo":
        return "Negativo", "Hecho adverso para la marca"
    if features["hedging"] > 1:
        return "Neutro", "Dato reportado o informativo"
...
class ClasificadorTonoUltraV2:
    ...
    async def _clasificar_grupo_async(self, texto_representante: str, semaphore: asyncio.Semaphore):
        async with semaphore:
            ...
            tono_h, just_h = decidir_tono(feats)
            return {"tono": tono_h, "justificacion": just_h}
...
def analizar_tono_con_pkl(textos: List[str], pkl_file: io.BytesIO) -> Optional[List[Dict[str, str]]]:
    try:
        ...
        st.error(f"❌ Error al procesar el archivo .pkl: {e}")
        st.warning("El pipeline debe ser un objeto Scikit-learn que implemente `.predict()`.")
        return None
...
def clasificar_temas_con_xlsx(textos: List[str], xlsx_file: io.BytesIO, columna_tema: str, p_bar) -> Optional[List[str]]:
    try:
        df_temas = pd.read_excel(xlsx_file)
        if columna_tema not in df_temas.columns:
            st.error(f"❌ La columna '{columna_tema}' no se encontró en el archivo Excel.")
            return None
        
        temas_predefinidos = df_temas[columna_tema].dropna().unique().tolist()
        if not temas_predefinidos: st.error("❌ No se encontraron temas válidos."); return None
        
        p_bar.progress(0.2, "📚 Obteniendo embeddings de temas predefinidos...")
        emb_temas = np.array([e for e in [get_embedding(t) for t in temas_predefinidos] if e is not None])
        
        p_bar.progress(0.4, "📰 Obteniendo embeddings de las noticias...")
        emb_noticias = np.array([e for e in [get_embedding(t) for t in textos] if e is not None])

        if emb_temas.shape[0] == 0 or emb_noticias.shape[0] == 0:
            st.error("❌ No se pudieron generar embeddings."); return ["Error"] * len(textos)

        p_bar.progress(0.7, "🤖 Asignando temas...")
        similitudes = cosine_similarity(emb_noticias, emb_temas)
        indices = np.argmax(similitudes, axis=1)
        
        p_bar.progress(1.0, text="✅ Temas asignados desde archivo.")
        return [temas_predefinidos[i] for i in indices]
    except Exception as e:
...
def consolidar_subtemas_en_temas(subtemas: List[str], p_bar) -> List[str]:
    ...
    for cluster_id, lista_subtemas in mapa_cluster_a_subtemas.items():
        ...
        try:
            resp = call_with_retries(openai.ChatCompletion.create, model=OPENAI_MODEL_CLASIFICACION, messages=[{"role": "user", "content": prompt}], max_tokens=20, temperature=0.1)
...
# ======================================
def are_duplicates(r1, r2, k, thresh=0.9, days=1):
    if norm_key(r1.get(k["menciones"])) != norm_key(r2.get(k["menciones"])) or norm_key(r1.get(k["medio"])) != norm_key(r2.get(k["medio"])): return False, ""
    tipo_medio = normalizar_tipo_medio(str(r1.get(k.get("tipodemedio"), "")))
    if tipo_medio.lower() in ["prensa", "diario", "revista", "revistas"]: return False, ""
    t1, t2 = normalize_title_for_comparison(str(r1.get(k["titulo"]))), normalize_title_for_comparison(str(r2.get(k["titulo"])))
    if not t1 or not t2: return False, ""
    if SequenceMatcher(None, t1, t2).ratio() >= thresh: return True, "second" if len(t2) > len(t1) else "first"
    return False, ""

def normalizar_tipo_medio(tipo_raw: str) -> str:
    t = unidecode(str(tipo_raw).strip().lower())
    mapping = {"fm": "Radio", "am": "Radio", "radio": "Radio", "aire": "Televisión", "cable": "Televisión", "tv": "Televisión", "prensa": "Prensa", "online": "Internet"}
    ...
    if rows_to_analyze:
        ...
        with st.status("🏷️ **Paso 3/3:** Análisis de Tema y Subtema", expanded=True) as s:
            ...
            if temas_xlsx_file:
                ...
                subtemas = clasif_temas.procesar_lote(df_temp["resumen_api"], p_bar_temas, df_temp[key_map["resumen"]], df_temp[key_map["titulo"]])
